...
def main():
    ...
    net.load_state_dict(torch.load(
        'saved_models/resnet18_gray_epochs65_bs64_vloss0.00491.pt'))
    ...
    out = cv2.VideoWriter('output_4.avi', cv2.VideoWriter_fourcc(
        'M', 'J', 'P', 'G'), 20, (640, 480))
    ...
    while (i):
        ...
        for (x, y, w, h) in faces_1:
            ...
            predicted_key_pts[:, 0] = predicted_key_pts[:,
                                                        0] * org_shape[0] / 224 + x
            predicted_key_pts[:, 1] = predicted_key_pts[:,
                                                        1] * org_shape[1] / 224 + y
            ...
            for (x_point, y_point) in zip(predicted_key_pts[:, 0], predicted_key_pts[:, 1]):
                cv2.circle(image_with_detections_1,
                           (x_point, y_point), 3, (0, 255, 0), -1)
